...
class ReturnScreen extends Component {
	...
	getItems(accountState){
		...
		firebase.database().ref("/return/"+accountState.uid).on('value', (snapshot) => {
			let items = [];
	...
	renderRow(item){
		...
		return(
				<TouchableOpacity onPress={()=>navigate("ReturnInfo",{key:item.key, item_name:item.item_name,quantity:item.quantity,borrower:item.borrower,note:item.note})}>
					<CardItem style={{marginBottom:5}} >
			          <ResponsiveImage source={require('../../img/logo.png')} initWidth="80" initHeight="80" style={{marginLeft:-10}}/>
						...
						<View style={{flexDirection:'column', marginRight:50}}>
				          ...
				          <Text>Lender: {item.borrower}</Text>
	...
	static navigationOptions = ({navigation}) => {
        ...
        return{
	        ...
	        <Button transparent onPress={()=>navigation.navigate("ReturnAdd")}>
