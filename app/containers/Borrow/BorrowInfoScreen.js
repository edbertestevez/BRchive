...
class BorrowInfoScreen extends Component {
    ...
    deleteRecord(navigation){
    	firebase.database().ref("/borrow/"+user_uid+"/"+del_key).remove();
    ...
    saveChanges(){
    	...
    	firebase.database().ref("/borrow/"+user_uid+"/"+key).set({
				...
				<Content padder>
					<Form>
		            	...
		            	<Item stackedLabel>
		              	...
		              	<Input editable = {this.state.isEditing} selectTextOnFocus={true} onChangeText={(borrower)=>this.setState({borrower})} placeholder="Enter name of borrower">{this.state.borrower}</Input>
		            	</Item>
