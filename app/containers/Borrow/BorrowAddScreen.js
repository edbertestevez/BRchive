...
const user_uid = '';
class BorrowAddScreen extends Component {
    ...
    addRecord(){
    	//create new key
    	let new_key = firebase.database().ref("/borrow/"+user_uid).push().getKey();

    	firebase.database().ref("/borrow/"+user_uid+"/"+new_key).set({
				...
				<Content padder>
					<Form>
		            	...
		            	<Item stackedLabel>
		              	...
		              	<Input selectTextOnFocus={true} onChangeText={(borrower)=>this.setState({borrower})} placeholder="Enter name of borrower">{this.state.borrower}</Input>
		            	</Item>
...
export default connect(mapStateToProps,mapDispatchToProps)(BorrowAddScreen);